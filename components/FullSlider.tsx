...
export default function FullSlider({ headline = "", cards, className = "" }) {
        ...
        <div ref={sliderRef} className="keen-slider mt-9">
          {cards.map((card, i) => (
            <motion.div variants={item} className="keen-slider__slide">
              <img src={card.img} className="mx-auto" />
            </motion.div>
          ))}
        </div>
      </motion.div>
      <div className="mx-auto flex h-7 space-x-16">
        {loaded && instanceRef.current && (
          <>
            <Selector
              headline="Google Search"
              src="/images/google64.png"
              onClick={(e) =>
